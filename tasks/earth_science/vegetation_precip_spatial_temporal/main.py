"""
AgentHLE Task Specification: Vegetation–Precipitation Spatial vs Temporal Sensitivity

Puzzle in ecology: why does vegetation respond much more strongly to precipitation
across space than over time?

The agent is given three research questions and a Data Availability Statement listing
potential data sources. It must:
1. Download NDVI/GPP, precipitation, and soil moisture data for CONUS.
2. Regrid all datasets to a common resolution and merge into a single NetCDF.
3. Compute spatial and temporal sensitivity of NDVI to precipitation by biome.
4. Compute spatial and temporal sensitivity of NDVI to soil moisture by biome.
5. Train a random forest to explain the space–time discrepancy (D_ppt) using
   environmental features, and report SHAP-based feature importance.
6. Produce three key scientific conclusions.

Evaluation is staged (6 points total, partial credit).
"""
...
class TaskConfig(GeneralTaskConfig):
    ...
    TASK_TAG: str = "vegetation_precip_spatial_temporal"
    ...
    GEE_ACCOUNT_EMAIL: str = os.environ.get(
        "GEE_ACCOUNT_EMAIL", "agenthle.sv@gmail.com"
    )
...
Time period: Determine the maximum overlapping temporal range among all datasets
you choose to use (e.g., SMAP starts in 2015, MODIS NDVI in 2000, PRISM goes
back further). Use only the years where all selected products have data.

Data Availability:
The following datasets may be useful (you must decide which to use and how):
  - SMAP L3 enhanced surface soil moisture (NSIDC, https://doi.org/10.5067/M20OXIZHY3RJ)
  - SMAP L4 root-zone soil moisture (NSIDC, https://doi.org/10.5067/KN96XNPZM4EG)
  - PRISM Precipitation (https://prism.oregonstate.edu/recent/)
  - MODIS MOD13A1 Vegetation Indices (LPDAAC, https://doi.org/10.5067/MODIS/MOD13A1.006)
  - MODIS MOD17A2H GPP (LPDAAC, https://doi.org/10.5067/MODIS/MOD17A2H.006)
  - IGBP land-cover classifications (https://doi.org/10.5067/MODIS/MCD12Q1.006)
  - TerraClimate for PET and Aridity Index (https://www.climatologylab.org/terraclimate.html)
  - Koppen-Geiger climate classification (https://www.gloh2o.org/koppen/)
  - Root zone water storage capacity (https://zenodo.org/records/5515246)
  - Soil properties from OpenLandMap (https://stac.openlandmap.org/)
  - Rainfall characteristics (https://zenodo.org/records/3605620, Bassiouni 2020)

Authentication (for Google Earth Engine access):
  Service account key is in the input folder:
...
3. feature_importance.csv
   ...
   mean_temporal_slope per pixel) using environmental features such as
   seasonal/interannual variability of precipitation and soil moisture,
   rainfall intensity/frequency, soil properties (bulk density), rooting
   depth, PET, aridity index, etc.
...
async def start(task_cfg, session: cb.DesktopSession):
    """Prepare the remote environment for the sensitivity analysis task."""
    ...
    try:
        try:
            if await session.exists(output_dir):
...
def _check_stage2_regrid(tmpdir: str, file_map: dict[str, str]) -> tuple[float, list[dict]]:
    ...
    try:
        with xr.open_dataset(file_map["merged.nc"]) as ds:
            ...
            first_var_shape = {
                d: ds.sizes[d] for d in spatial_dims_per_var[0]
            }
            checks.append({
...
LOCAL_REFERENCE_DIR = Path(__file__).resolve().parent / "reference"

NUMERIC_COLS_TO_COMPARE = [
    "spatial_slope", "spatial_r2", "mean_temporal_slope",
    "temporal_r2_mean", "difference", "relative_difference",
]
...
def _check_sensitivity_csv(
    ...
    expected_cols = {
        "biome", "spatial_slope", "spatial_r2", "mean_temporal_slope",
        "temporal_min", "temporal_max", "temporal_r2_mean",
        "difference", "relative_difference", "N",
    }
        ...
        if ref_path.exists():
            try:
                ...
                ref_checks = _compare_against_reference(
                    df, ref_df, stage_prefix, required_biome_keywords,
                )
...
def _check_stage5_feature_importance(
    ...
    water_var_count = sum(
        1 for feat in top3
        if any(kw in feat for kw in WATER_VARIABILITY_KEYWORDS)
    )
...
async def _llm_judge_conclusion(
    ...
    """Use an LLM to judge whether an agent's conclusion matches the expected finding.
    ...
    try:
        ...
        logger.warning(f"LLM judge failed for conclusion {conclusion_idx + 1}, "
...
async def _check_stage6_conclusions(
    ...
    for i in range(3):
        ...
        score, check = await _llm_judge_conclusion(
            agent_text, EXPECTED_CONCLUSIONS[i], i
        )
        checks.append(check)
        total_score += score
...
async def evaluate(task_cfg, session: cb.DesktopSession) -> list[float]:
    ...
    required_biome_keywords = metadata.get("required_biome_keywords", ["all", "grass", "savanna", "shrub"])
    ...
    async with EvaluationContext(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for fname in all_expected:
                ...
                if actual_name:
                    remote_path = fr"{output_dir}\{actual_name}"
                    try:
            ...
            score, checks = _check_stage5_feature_importance(file_map, min_fi_r2)
            total_points += score
            all_checks.extend(checks)
            ctx.add_score(score)
